...
void AShooterProjectile::BeginPlay()
{
	...
	UE_LOG(LogOnlineFPS, Warning, TEXT("AShooterProjectile::BeginPlay - %s, HasAuthority: %s, Instigator: %s, Owner: %s"),
		*GetName(), HasAuthority() ? TEXT("true") : TEXT("false"),
		*GetNameSafe(GetInstigator()), *GetNameSafe(GetOwner()));
...
{
	UE_LOG(LogOnlineFPS, Warning, TEXT("AShooterProjectile::ProcessHit - HitActor: %s, Instigator: %s, Owner: %s, HitDamage: %.2f, HasAuthority: %s"),
		*GetNameSafe(HitActor), *GetNameSafe(GetInstigator()), *GetNameSafe(GetOwner()), HitDamage, HasAuthority() ? TEXT("true") : TEXT("false"));
	...
	{
		// ignore the owner of this projectile's instigator (the character who shot)
		APawn* ShooterPawn = Cast<APawn>(GetInstigator());
		if (HitCharacter != ShooterPawn || bDamageOwner)
		{
			UE_LOG(LogOnlineFPS, Warning, TEXT("  Applying damage to %s"), *GetNameSafe(HitCharacter));
			// apply damage to the character
			UGameplayStatics::ApplyDamage(HitCharacter, HitDamage, GetInstigator()->GetController(), this, HitDamageType);
		}
		else
		{
			UE_LOG(LogOnlineFPS, Warning, TEXT("  Skipping damage - hit self"));
		}
	}
