...
public:
	...
	/** Restarts the AI logic (used after respawn) */
	void RestartAILogic();
...
private:
	...
	/** Current wander target for fallback AI */
	TOptional<FVector> CurrentWanderTarget;

	/** Timer handle for fallback AI logic */
	FTimerHandle FallbackAITimer;

	/** Starts fallback AI logic when StateTree fails to initialize */
	void StartFallbackAILogic(class AShooterNPC* NPC);

	/** Stops fallback AI logic */
	void StopFallbackAILogic();
};
